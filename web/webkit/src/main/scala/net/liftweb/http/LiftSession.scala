...
class LiftSession(private[http] val _contextPath: String, val uniqueId: String,
  ...
  */
   ...
   * The // HERE
   */
  def buildRoundtrip(info: Seq[RoundTripInfo]): JsExp = buildRoundtrip(() => RoundTripCometActor())(info)

  def buildRoundtrip[RoundtripActorType <: CometActor](newActorFunc: () => RoundtripActorType)(info: Seq[RoundTripInfo], afterCreationProcess: RoundtripActorType => Unit = (actor: RoundtripActorType) => {}): JsExp = {
    testStatefulFeature{
